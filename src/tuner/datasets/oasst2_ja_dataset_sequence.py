...
OVERWRITE_RECOMMENDED_OPTIONS = True
...
def get_oasst2_ja_dataset_sequence(
    ...
    *,
    ...
    """
    Args:
        dataset_config (dataclass): DatasetConfig object
        tokenizer (AutoTokenizer): tokenizer
        format_fn (BasePrompt): format function
        model_max_length (int, optional): model max length
        split (str, optional): split
        verbose (bool, optional): verbose

    Returns:
        ChatDataset: a custom chat dataset
    ...
    # Make a copy of the dataset_config to avoid modifying the original object
    dataset_config = replace(dataset_config)

    if OVERWRITE_RECOMMENDED_OPTIONS and split in ('eval', 'test') and dataset_config.augmentation:
        print_warning_on_rank_0(
            f'You specified `augmentation=True` for {split} split. '
            f'Since this is not recommended, `augmentation` is set to `False`. '
            f'To disable overwriting, set `overwrite_recommended_options=False`.',
        )
        dataset_config.augmentation = False

    if OVERWRITE_RECOMMENDED_OPTIONS and split in ('eval', 'test') and dataset_config.shuffle:
        print_warning_on_rank_0(
            f'You specified `shuffle=True` for {split} split. '
            f'Since this is not recommended, `shuffle` is set to `False`. '
            f'To disable overwriting, set `overwrite_recommended_options=False`.',
        )
